...
const ComposeMomentScreen = ({ navigation }: Props) => {
    ...
    for (const asset of result.assets) {
      ...
      (async () => {
        try {
          ...
          const fileName = asset.fileName || `moment-${id}.jpg`;
  ...
  const handlePost = async () => {
    ...
    setSubmitting(true);
    try {
      await momentService.create({ content: trimmed, attachments });
      navigation.goBack();
    } catch (err: any) {
      Alert.alert(
        t('common.failed'),
...
export default ComposeMomentScreen;
